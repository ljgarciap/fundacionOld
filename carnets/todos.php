...
$query="SELECT r.documentor,upper(concat(r.nombresr,' ',r.apellidosr)) as nombre,r.fechanacimiento,r.eps,r.nomfund,
h.fechaingreso,(year(now())-year(r.fechanacimiento)) as edad,upper(concat(u.nombres,' ',u.apellidos)) as acudiente 
from residentes r join historial h on r.idresidentes=h.idresidentes join asociacion a on r.idresidentes=a.idresidentes 
join usuarios u on a.idusuarios=u.idusuarios where r.estado='A' or r.estado='E' order by nombresr";
